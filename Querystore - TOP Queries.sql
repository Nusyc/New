...
@Top = 25
, @TopBy = 'CPU'
, @DatetimeInicioLocal = '20221220 00:00'
, @DatetimeFimLocal = '20230101 00:00'
, @DatabasesSeparadasPorVirgula = 'ALL'
, @OutputTable = 'QsTopQueriesCPU_V2'
...
#DBS b
WHERE
a.name collate Latin1_General_CI_AI = b.db collate Latin1_General_CI_AI
)
...
WHERE /*filtro passado anteriormente na tab temporaria*/
   NOT ((DATEADD(MI, DATEDIFF(MI, GETUTCDATE(), GETDATE()),rs.first_execution_time) > @DatetimeFimLocal OR DATEADD(MI, DATEDIFF(MI, GETUTCDATE(), GETDATE()),rs.last_execution_time ) < @DatetimeInicioLocal))
...
, QuerySample = NULL' + -- + CASE when @compatibility_level <= 100 THEN '''-- Info - Consulting QuerySampleText column to get it''' ELSE 'dbo.fn_QueryTextToXML_TopQueryStoreQuery(Tab2.query_sql_text)' END + N'
N', QuerySampleText = Tab2.query_sql_text
